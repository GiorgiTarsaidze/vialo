...
The active endpoints are `https://ap9i8up7k7.execute-api.us-east-1.amazonaws.com` and `https://api.vialo.place`. The custom hostname is a Cloudflare DNS-only CNAME to the REGIONAL API Gateway domain; live verification confirmed DNS resolution, hostname-matched ACM coverage for `*.vialo.place`, TLS 1.3 negotiation under the AWS TLS 1.2 minimum policy, and the `$default` API mapping.
